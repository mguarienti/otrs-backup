...
# --
# Implementado envio de e-mails quando os respectivos laços dispararem erros. Verificar README
# --
...
if ( !$Opts{d} ) {
    ...
    system ( 'echo "Erro na etapa de checagem de diretório: Need -d for backup directory - Linha 51." 
    | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
    exit 1;
}
elsif ( !-d $Opts{d} ) {
    print STDERR "ERROR: No such directory: $Opts{d}\n";
    system ( 'echo "Erro na etapa de checagem de diretório: No such directory - Linha 57." 
    | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
    exit 1;
}
...
if ( $DatabaseDSN =~ m/:mysql/i ) {
    ...
    system ( 'echo "Erro na etapa de checagem de suporte ao banco de dados: Cant backup, no database dump support! - Linha 119." 
    | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
    exit(1);
}
...
for my $CMD ( 'cp', 'tar', $DBDump, $CompressCMD ) {
    ...
    if ( !$Installed ) {
        ...
        system ( 'echo "Erro na etapa de checagem de programas necessários: Cant locate - Linha 134." 
        | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
        exit 1;
    }
}
...
if ( !mkdir($Directory) ) {
    system ( 'echo "Erro na etapa de criação do nome do diretório: Cant create directory - Linha 158." 
    | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
...
if ( !system("tar -czf $Directory/Config.tar.gz Kernel/Config*") ) {
    ...
    system ( 'echo "Falha em backup de arquivo: Kernel/Config.pm - Linha 170." 
    | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
    die "Backup failed\n";
}
...
if ($DBOnlyBackup) {
    ...
    system ( 'echo "Falha em backup: Backup of filesystem data disabled by parameter dbonly - Linha 178." 
    | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
}
else {
    if ($FullBackup) {
        ...
        if ( !system("tar $Excludes -czf $Directory/Application.tar.gz .") ) {
            ...
            print "failed\n";
            RemoveIncompleteBackup($Directory);
            system ( 'echo "Falha em backup: Backup application - Remove Incomplete Backup Failed - Linha 192." 
            | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
            die "Backup failed\n";
        }
    }
    ...
    # backup vardir
        ...
        if ( !system("tar -czf $Directory/VarDir.tar.gz var/") ) {
            ...
            print "failed\n";
            RemoveIncompleteBackup($Directory);
            system ( 'echo "Falha em backup de diretório: Backup vardir error Remove Incomplete Backup - Linha 208." 
            | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
            die "Backup failed\n";
        }
    }
    ...
    if ( $ArticleDir !~ m/\A\Q$Home\E/ ) {
        ...
        if ( !system("tar -czf $Directory/DataDir.tar.gz $ArticleDir") ) {
            ...
            print "failed\n";
            RemoveIncompleteBackup($Directory);
            system ( 'echo "Falha em backup de diretório: Backup datadir error Remove Incomplete Backup - Linha 222." 
            | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
            die "Backup failed\n";
        }
    }
...
if ( $DB =~ m/mysql/i ) {
    ...
    if ( !system("$DBDump -u $DatabaseUser $DatabasePw -h $DatabaseHost $Database > $Directory/DatabaseBackup.sql") ) {
        ...
        print "failed\n";
        RemoveIncompleteBackup($Directory);
        system ( 'echo "Falha em backup: Backup database - laço 1 - Remove Incomplete Backup Failed - Linha 241." 
        | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
        die "Backup failed\n";
    }
}
    ...
    if ( !system("$DBDump -f $Directory/DatabaseBackup.sql $DatabaseHost -U $DatabaseUser $Database") ) {
        ...
        print "failed\n";
        RemoveIncompleteBackup($Directory);
        system ( 'echo "Falha em backup: Backup database - laço 2 - Remove Incomplete Backup Failed - Linha 263." 
        | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
        die "Backup failed\n";
    }
}
...
if ( !system("$CompressCMD $Directory/DatabaseBackup.sql") ) {
    print "done\n";
    # DESCOMENTAR ABAIXO PARA ENVIO DE E-MAIL QUANDO BACKUP FOI FINALIZADO COM SUCESSO
    # system ( 'echo "Backup finalizado com sucesso" | mail -s "Backup do OTRS Finalizado" seu@email.com.br' );
}
else {
    print "failed\n";
    RemoveIncompleteBackup($Directory);
    system ( 'echo "Falha em backup: Compressing database failed - Remove Incomplete Backup Failed - Linha 279." 
    | mail -s "Erro no Backup do OTRS" seu@email.com.br' );
    die "Backup failed\n";
}
    ...
    for my $Directory (@Directories) {
        ...
        if ( !$Leave ) {
            ...
            if ( rmdir($Directory) ) {
                ...
                system ( 'echo "Backup do OTRS foi concluido mas falhou ao apagar backups antigos - Linha 351" 
                | mail -s "Erro em backup do OTRS" seu@email.com.br' );
                die "failed\n";
            }
        }
    }
